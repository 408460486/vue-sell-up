<template>
  <div class="goods">
    <div class="view-wrapper">
      <cube-scroll-nav :side="true" :data="goods" :options="scrollOptions" v-if="goods.length" @change="changeHandler" @sticky-change="stickyChangeHandler">
        <template slot="bar" slot-scope="props">
          <cube-scroll-nav-bar direction="vertical" :labels="props.labels" :txts="barTxts" :current="props.current">
            <template slot-scope="props">
              <div class="text">
                <support-ico v-if="props.txt.type >= 1" :type="props.txt.type" :size="3"></support-ico>
        ...
        <cube-scroll-nav-panel v-for="good in goods" :key="good.name" :label="good.name" :title="good.name">
          <ul>
            <li v-for="(food, index) in good.foods" :key="index" class="food-item" @click="showFood(food)">
              ...
              <div class="car-control-warpper"><car-control :food="food" @add="drop"></car-control></div>
            </li>
          </ul>
        </cube-scroll-nav-panel>
      </cube-scroll-nav>
    </div>
    <shop-car-list ref="shopCarList" :selected-foods="selectedFoods" @maskClick="maskClick" @add="drop"></shop-car-list>
    <shop-car
      ref="shopCar"
      ...
      @listEmpty="listEmpty"
    ></shop-car>
    <transition name="move"><div v-show="foodMaskVisible" class="food-details-mask"></div></transition>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    showFood(food) {
      this.$bus.$emit(this.$event.FOOD_VISIBLE, food)
    },
    _showFoodMask(flag) {
